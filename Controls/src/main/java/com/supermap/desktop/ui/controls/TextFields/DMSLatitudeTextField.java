...
/**
 * Created by yuanR on 2017/10/17 0017.
 * * 支持输入度分秒的组合控件（纬度版）
 * 组合控件由三个TextField和三个Label组合而成，
 */
public class DMSLatitudeTextField extends JPanel {
	...
	private KeyListener keyAdapter = new KeyAdapter() {
		...
		public void keyTyped(KeyEvent e) {
			...
			if (e.getSource().equals(textFieldM)) {
				...
				// 最多输入两个数字
				if (textD.contains("-") && textD.length() >= 3 || !textD.contains("-") && textD.length() >= 2) {
					e.consume();
				}

				// 当且仅当分、秒都为0时，才可以输入90
				if (textD.length() >= 1 && StringUtilities.getNumber(textD.replace("-", "")) == 9) {
	...
	private ISmTextFieldLegit iSmTextFieldLegitDegree = new ISmTextFieldLegit() {
		...
		public boolean isTextFieldValueLegit(String textFieldValue) {
			...
			try {
				...
				if ((!minute.equals(0) || !second.equals(0)) && integer == 90) {
					return false;
				}
	...
	private void initComponent() {
		...
		this.textFieldD.setToolTipText(ControlsProperties.getString("String_RangeSection") + "[-90,90]");
